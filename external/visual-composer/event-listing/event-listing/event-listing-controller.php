<?php
namespace WPEMEventListing\eventListing;
    ...
    class Event_Listing_Controller extends Container implements Module
    {
        use EventsFilters;
        use WpFiltersActions;
        public function __construct()
        {
            if (!defined('VCV_WPEM_CAT_CONTROLLER')) {
                $this->addFilter(
                ...
                define('VCV_WPEM_CAT_CONTROLLER', true);
            }
            if (!defined('VCV_WP_TYPE_CONTROLLER')) {
                ...
                define('VCV_WPEM_TYPE_CONTROLLER', true);
            }
            if (!defined('VCV_WPEM_DATE_CONTROLLER')) {
                $this->addFilter(
                    'vcv:editor:variables vcv:editor:variables/eventListing',
                    'get_date_filter'
                );
                define('VCV_WPEM_DATE_CONTROLLER', true);
            }
            if (!defined('VCV_WPEM_TICKET_CONTROLLER')) {
                $this->addFilter(
                    'vcv:editor:variables vcv:editor:variables/eventListing',
                    'get_ticket_type'
                );
                define('VCV_WPEM_TICKET_CONTROLLER', true);
            }
            if (!defined('VCV_WPEM_AUTOCOMPLETE_CATEGORY')) {
                $this->addFilter('vcv:autocomplete:listCategory:render', 'get_category_autocomplete_suggester');
                define('VCV_WPEM_AUTOCOMPLETE_CATEGORY', true);
            }
            if (!defined('VCV_WPEM_AUTOCOMPLETE_TYPE')) {
                $this->addFilter('vcv:autocomplete:listType:render', 'get_type_autocomplete_suggester');
                define('VCV_WPEM_AUTOCOMPLETE_TYPE', true);
            }
        }
        /**
         ...
         * @since 3.1.8
         */
        protected function get_type_autocomplete_suggester($response, $payload)
        {
            global $wpdb;
         ...
         * @since 3.1.8
         */
        protected function get_category_autocomplete_suggester($response, $payload)
        {
            global $wpdb;
            $searchValue = $payload['searchValue'];
            $returnValue = $payload['returnValue'];
         ...
         * @since 3.1.8
         */
        protected function get_category($variables)
        {
            $args = array(
                'orderby'                  => 'name',
                'order'                    => 'ASC',
         ...
         * @since 3.1.8
         */
        protected function get_type($variables)
        {
            $args = array(
                'orderby'                  => 'name',
                'order'                    => 'ASC',
         ...
         * @since 3.1.8
         */
        protected function get_date_filter($variables)
        {
         ...
         * @since 3.1.8
         */
        protected function get_ticket_type($variables)
        {
